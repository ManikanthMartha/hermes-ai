...
import { logger, type WorkspaceContext } from "@hermes/shared";
...
type ClientEntry = {
  ...
  toolsCache: Map<string, StructuredToolInterface> | null;
};

const clientEntries = new Map<string, ClientEntry>();

function getClient(context: WorkspaceContext): ClientEntry {
  const key = cacheKey(context);
  const existing = clientEntries.get(key);
  if (existing) return existing;

  const headers = {
    "x-hermes-runtime-secret":
      process.env.MCP_GATEWAY_SECRET ?? process.env.API_SECRET_KEY ?? "",
    "x-hermes-user-id": context.userId,
    "x-hermes-workspace-id": context.workspaceId,
  };
  ...
  const client = new MultiServerMCPClient({
    ...
    mcpServers: {
      slack: {
        url: process.env.MCP_SLACK_URL ?? "http://127.0.0.1:4110/slack/mcp",
        headers,
        reconnect: { enabled: true, maxAttempts: 5, delayMs: 2000 },
      },
      github: {
        url: process.env.MCP_GITHUB_URL ?? "http://127.0.0.1:4110/github/mcp",
        headers,
        reconnect: { enabled: true, maxAttempts: 5, delayMs: 2000 },
      },
      linear: {
        url: process.env.MCP_LINEAR_URL ?? "http://127.0.0.1:4110/linear/mcp",
        headers,
        reconnect: { enabled: true, maxAttempts: 5, delayMs: 2000 },
      },
      sentry: {
        url: process.env.MCP_SENTRY_URL ?? "http://127.0.0.1:4110/sentry/mcp",
        headers,
        reconnect: { enabled: true, maxAttempts: 5, delayMs: 2000 },
      },
      gmail: {
        url: process.env.MCP_GMAIL_URL ?? "http://127.0.0.1:4110/gmail/mcp",
        headers,
        reconnect: { enabled: true, maxAttempts: 5, delayMs: 2000 },
      },
    },
...
export async function getToolsForSpecialist(
  ...
  const all = await loadAllTools(context);
...
function cacheKey(context: WorkspaceContext) {
